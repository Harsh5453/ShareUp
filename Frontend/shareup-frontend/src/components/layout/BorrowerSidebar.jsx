...
import { FaSearch, FaClipboardList, FaStar, FaSignOutAlt } from 'react-icons/fa'

const links = [
  { to: '/borrower/browse',  label: 'Browse Items', icon: <FaSearch />,        desc: 'Find items to rent' },
  { to: '/borrower/rentals', label: 'My Rentals',   icon: <FaClipboardList />, desc: 'Track your rentals' },
  { to: '/borrower/ratings', label: 'Rate Owners',  icon: <FaStar />,          desc: 'Leave feedback' },
]
  ...
  return (
    <>
      <style>{`
        ...
        .bsidebar-nav-item .item-label {
          font-size: 0.875rem;
          font-weight: 500;
          color: #9ca3af;
          transition: color 0.2s;
        }
        .bsidebar-nav-item:hover .item-icon {
          background: rgba(59,130,246,0.15);
          color: #3b82f6;
        }
        .bsidebar-nav-item:hover .item-label { color: #fff; }
        .bsidebar-nav-item.active .item-icon {
          background: #3b82f6;
          color: white;
        }
        .bsidebar-nav-item.active .item-label { color: #fff; font-weight: 600; }
        .bsidebar-nav-item.active::before {
          content: '';
          position: absolute;
          left: 0; top: 50%;
        ...
        <nav style={{ flex: 1, padding: '0 12px' }}>
          {links.map(link => (
            <NavLink
              key={link.to}
              to={link.to}
              className={({ isActive }) => `bsidebar-nav-item ${isActive ? 'active' : ''}`}
            >
        ...
        <div>
          ...
          <div className="b-user-card">
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div className="b-user-avatar">{user?.email?.[0]?.toUpperCase()}</div>
